...
echo "docker run -d -v ${VOLMOUNT} --name appprd -p 24200:4200  angulardev /bin/sh -c \"cd /nodeapp && npm install -d && ./start-poc.sh\""
docker run -d -v ${VOLMOUNT} --name appprd -p 24200:4200 angulardev /bin/sh -c "cd /nodeapp && npm install -d && ./start-poc.sh"
